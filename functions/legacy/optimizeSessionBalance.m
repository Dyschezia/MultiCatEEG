function Experiment = optimizeSessionBalance(Experiment)
 
nshuffles = 100;
nreps = 100;

nSessions = 3;
nPerms = size(Experiment.DM.samplesPermutation, 1);
nCombs = size(Experiment.DM.samplesCombination, 1);
runLength = nPerms/nSessions;
runCombRep = runLength/nCombs;

% First create the starting point runs matrix.
runs = zeros(runLength,4,nSessions);
for ci = 1:length(Experiment.DM.samplesCombination)

    idx = (ci-1) * runCombRep + 1; % Always store 8 permutations together
    idxend = idx + runCombRep - 1;

    c = Experiment.DM.samplesCombination(ci, :);
    thiscperms = perms(c);
    [r,c] = size(thiscperms);
    nlay = nSessions; % Number of layers (=runs)
