...
NUM_SESSIONS = 6  # how many sessions are we interested in today?
NUM_BLOCKS_PER_SES = 4  # how many blocks are there in each session (runs?)
GLOBAL_LOCATION_DICTS = [{x: 0 for x in range(1, NUM_CATEGORIES + 1)} for y in range(NUM_SIMULT_PRES)]
SESSION_LOCATION_DICTS = [[{x: 0 for x in range(1, NUM_CATEGORIES + 1)} for y in range(NUM_SIMULT_PRES)] for y in
                          range(NUM_SESSIONS)]
CURRENT_SESSION = 0  # This is a trick in order to use only ONE compare_by_locations_session function
...
def compare_by_locations_session(tup1, tup2):
    """
    Given two tuples of NUM_SIMULT_PRES elements each, it returns 1 if tup1 > tup2, and -1 if tup1 < tup2,
    ...
    :return: 1 or -1 according to the larger tuple; if equal arbitrarily choose one as larger
    """
    tup1_list = [NumberCounter(tup1[x], SESSION_LOCATION_DICTS[CURRENT_SESSION][x][tup1[x]]) for x in range(len(tup1))]
    tup2_list = [NumberCounter(tup2[x], SESSION_LOCATION_DICTS[CURRENT_SESSION][x][tup2[x]]) for x in range(len(tup2))]
    tup1_list.sort(reverse=False, key=lambda x: x.counter)
    tup2_list.sort(reverse=False, key=lambda x: x.counter)
...
def combinations_in_sessions():
    ...
    sessions_tuples = [list() for _ in
                       range(NUM_SESSIONS)]  # open a list of sessions (each is a list of tuples (combinations))
    num_permutations_per_combination = len(
        list(itertools.permutations([x for x in range(1, NUM_SIMULT_PRES + 1)], NUM_SIMULT_PRES)))
    ...
    for i in range(int(num_combinations)):  # for each combination
        combinations_sorted = sorted(combinations,
                                     key=functools.cmp_to_key(compare))  # sort combinations by the comparison method
        selected_tuple = combinations_sorted[
            0]  # choose the smallest, which is the combination of the least used categories
        # combinations.remove(selected_tuple)
        for num in selected_tuple:  # update the global dictionary CAT_SELECT_DICT
            CAT_SELECT_DICT[num] += int(num_permutations_per_combination / NUM_SESSIONS)
        for j in range(int(num_permutations_per_combination)):
            # Now, need to choose PERMUTATION
            session = j % NUM_SESSIONS
            # We MUST set CURRENT_SESSION before using compare_by_locations_session, because it uses it to find the correct session
            CURRENT_SESSION = session
            permutations_sorted = sorted(all_permutations_of_combination[selected_tuple], key=functools.cmp_to_key(compare_by_locations_session))  # sort combinations by the comparison method
            ...
            for x in range(len(selected_permutation)):  # update the global dictionary GLOBAL_LOCATION_DICTS
                SESSION_LOCATION_DICTS[session][x][selected_permutation[x]] += 1
            all_permutations_of_combination[selected_tuple].remove(selected_permutation)
            sessions_tuples[session].append(selected_permutation)  # put it in the corresponding session
    ...
    for session_list in sessions_tuples:
        print(f"Session {i} number of trials: {len(session_list)}")
        ...
        for tup in session_list:
            ...
            comb = tuple(sorted(list(tup)))
            if comb not in combination_counter:
                combination_counter[comb] = 0
            combination_counter[comb] += 1
        is_ok = True
        for comb in combination_counter.keys():
            if combination_counter[comb] != int(num_permutations_per_combination / NUM_SESSIONS):
                is_ok = False
        if is_ok:
            print(
                f"Session {i} has exactly {int(num_permutations_per_combination / NUM_SESSIONS)} permutations of each combination")
        else:
            print(
                f"ERROR!!!!! Session {i} does not have exactly {int(num_permutations_per_combination / NUM_SESSIONS)} permutations of each combination")
...
def permutations_in_sessions(sessions_tuples):
    ...
    NOTE that this does not consider location.
    ...
    for session in sessions_tuples:
        ...
        for i in range(num_permutations):  # for each combination
            combinations_sorted = sorted(session_mega_list, key=functools.cmp_to_key(
                compare))  # sort combinations by the comparison method
            selected_tuple = combinations_sorted[
                0]  # choose the smallest, which is the combination of the least used categories
